...
export async function onRequestPost(context) {
  ...
  const pushCompany = (c, tab, section) => {
    ...
    statements.push(env.DB.prepare(`
      INSERT INTO companies (id, tab, section, name, li, hr, em, wa, land, note, job_url, job_title, source, scraped_from, created_at)
    ...
    statements.push(env.DB.prepare(`
      INSERT INTO progress (company_id, stage, note, updated_at) VALUES (?1,'none','',?2)
      ON CONFLICT(company_id) DO NOTHING
    `).bind(id, now));
    companiesAdded++;

    if (c.stage && VALID_STAGES.includes(c.stage) && c.stage !== "none") {
      statements.push(progressUpsert(env, id, c.stage, c.stage_note || "", c.stage_at || now));
      progressMerged++;
    }
  };
  ...
  if (body.state && typeof body.state === "object") {
    ...
    for (const [oldId, rec] of Object.entries(body.state)) {
      ...
      for (const id of matches) {
        statements.push(progressUpsert(env, id, rec.stage, rec.note || "", rec.at || now));
        progressMerged++;
      }
    }
  }
  ...
  return json({ companies_added: companiesAdded, progress_merged: progressMerged });
}

/** Newest-wins: only overwrite if the incoming timestamp is at least as new. */
function progressUpsert(env, id, stage, note, at) {
